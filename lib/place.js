...
Place.prototype.can_block_mills = function () {
  return this.how_many_forms_mill(true);
}

Place.prototype.how_many_forms_mill = function(is_other_player) {
  var player = is_other_player == true?this.game.other_player(this.game.current_player):this.game.current_player;
  return this.forms_mill_with(player).length;
};

Place.prototype.forms_mill_with = function(player) {
  ...
  for (var i = 0; i < mill_places.length; i++) {
    ...
    if (line_complete) {
      places_friends.push(place);
      break;
    }
  }
  return places_friends;
};
