...
Game.prototype.check_win = function() { // to do at the end of a turn
  if (this.state === GameState.FIRST_STAGE) {
    return null;
  }
  ...
  var previous_player = this.current_player;
  var next_player = this.other_player(previous_player);

  // current player has 2 pieces left or can't move
  if(next_player.pieces_on_board().length <= 2 || !next_player.can_move()) {
